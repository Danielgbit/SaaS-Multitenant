...
async function testDirectChannel() {
  ...
  if (result.mockPayload) {
    const mp = result.mockPayload as any
    console.log(`  Payload endpoint: ${mp.endpoint}`)
    console.log(`  Payload to: ${mp.to}`)
    console.log(`  Payload text preview: ${mp.text?.substring(0, 80)}...`)
    console.log(`  Timestamp: ${mp.timestamp}`)
  }
}
