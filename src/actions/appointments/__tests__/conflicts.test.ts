...
vi.mock('@/lib/auth/require-org-access', () => ({
  requireOrgAccess: vi.fn().mockResolvedValue({ success: true, context: { userId: 'u1', organizationId: 'org-1', role: 'admin' } }),
}))
...
const { validateCreateInput, checkCreatePreconditions, verifySlotAvailability, computeAppointmentTimes } = await import('@/lib/appointments/create-appointment-core')
...
describe('conflictos', () => {
  ...
  it('rechaza por conflicto de horario', async () => {
    vi.mocked(verifySlotAvailability).mockResolvedValue({ success: false, error: 'El empleado ya tiene una cita en este horario.' })
    const result = await createAppointment(VALID())
    expect(result.error).toContain('horario')
  })

  it('rechaza cuando empleado no existe', async () => {
    vi.mocked(checkCreatePreconditions).mockResolvedValueOnce({ success: false, error: 'Empleado no encontrado.' })
    const result = await createAppointment(VALID())
