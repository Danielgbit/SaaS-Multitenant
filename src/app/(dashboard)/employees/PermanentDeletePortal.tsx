...
interface PermanentDeletePortalProps {
...
export function PermanentDeletePortal({ employee, onClose }: PermanentDeletePortalProps) {
  ...
  useEffect(() => {
    ...
    let mounted = true
    countEmployeeRecords(employee.id).then((counts) => {
      if (!mounted) return; setRecordCounts(counts); setIsLoadingCounts(false)
    }).catch(() => {
      if (!mounted) return
      setRecordCounts({ appointments: 0, confirmations: 0, availability: 0, services: 0, loans: 0, receipts: 0, hasActiveAppointments: false })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (!result.success && result.error) setError(result.error); else onClose()
    })
  }

  const isConfirmValid = confirmText === 'ELIMINAR'
  const recordKeys: (keyof Omit<EmployeeRecordCounts, 'hasActiveAppointments'>)[] = ['appointments', 'confirmations', 'availability', 'services', 'loans', 'receipts']
  const itemsWithCount = recordCounts ? recordKeys.filter(key => recordCounts[key] > 0).map(key => [key, recordCounts[key]] as [typeof key, number]) : []
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Eliminar permanentemente">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="portal-delete-confirm" className="text-xs font-semibold text-[#475569] dark:text-[#94A3B8]">Escribe "ELIMINAR" para confirmar</label>
          <input id="portal-delete-confirm" type="text" value={confirmText} onChange={(e) => setConfirmText(e.target.value)}
            placeholder="ELIMINAR" autoComplete="off"
            className="w-full mt-1 px-3 py-2 rounded-xl border-2 border-[#E2E8F0] dark:border-[#475569] bg-white dark:bg-[#111827] text-center font-mono text-base uppercase tracking-wider" />
        </div>
        <div className="flex gap-3 pt-2">
          <Button variant="secondary" onClick={onClose} className="flex-1">Cancelar</Button>
          <Button type="submit" variant="danger" disabled={!isConfirmValid || isPending || recordCounts?.hasActiveAppointments || isLoadingCounts} loading={isPending} className="flex-1">Eliminar</Button>
        </div>
      </form>
    </Modal>
  )
